...
class sef_content {
	...
	function revert( &$url_array, $pos ) { // V 1.2.4.l  // updated based on includes/sef.php.
		$url_array = array_filter( $url_array ); // V x : traling slash can cause empty array element
		$url_array = array_values( $url_array );
...
class sef_404 {
			...
			switch ( $view ) {
				case 'category':
					if ( empty( $layout ) || $layout == 'default' ) {
						if ( ! empty( $Itemid ) ) {
							$menuItem = \Joomla\CMS\Factory::getApplication()->getMenu( 'site' )->getItem( $Itemid );
						}
						if ( ! empty( $menuItem )
						     && $menuItem->component == 'com_content'
						     && isset( $menuItem->query['view'], $menuItem->query['id'] )
						     && $menuItem->query['view'] == 'category'
						     && $menuItem->query['id'] == $id ) {
							if ( isset( $menuItem->query['layout'] ) ) {
								$layout = $menuItem->query['layout'];
							}
						}

						if ( empty( $layout ) ) {
							$categories = JCategories::getInstance( 'content', $options = array( 'access' => false, 'published' => 0 ) );
							$category   = $categories->get( $id );
							if ( empty( $category ) ) {
								$layout = 'default';
							} else {
								$cparams          = $category->getParams();
								$category->params = clone \Joomla\CMS\Factory::getApplication()->getParams();
								$category->params->merge( $cparams );
								$paramsLayout = $category->params->get( 'category_layout' );
								if ( wbContains( $paramsLayout, ':' ) ) {
	 ...
	 * @return sefurl
	 */
	public static function sefGetLocation(
		$nonSefUrl, &$title, $task = null, $limit = null
				...
				if ( $finalSefUrl != '' ) {
					...
					if ( $nonSefUrlType != sh404SEF_URLTYPE_NONE && $nonSefUrlType != sh404SEF_URLTYPE_404 ) {
						...
						if ( is_null( $dburl ) || ( $isDuplicate && $sefConfig->shRecordDuplicates ) ) {
							...
							ShlDbHelper::insert( '#__sh404sef_urls', array(
								'oldurl'  => $finalSefUrl,
								'newurl'  => $nonSefUrl,
								'rank'    => $rank,
								'dateadd' => $dateAdd
