    ...
    public function validateResponse($url, $response)
    {
        ...
        if (strpos($url, '::(~)') === false && $response->code != $code)
        {
            if ($error = json_decode($response->body))
            {
                ...
                $this->responseError = 'Code ' . $code . ' received with message: ' . $message . '.';
            }
            else
            {
                $this->responseError = $response->body;
            }

            if (JFBCFactory::config()->get('facebook_display_errors') && $this->responseError)
                JFBCFactory::log('Twitter API Error' . $this->responseError, 'error');
