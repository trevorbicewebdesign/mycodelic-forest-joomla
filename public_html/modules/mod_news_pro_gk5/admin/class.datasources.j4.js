...
DataSources.prototype.changeValue = function() {
	...
	jQuery($this.datasources).each(function(i, obj) {
		...
		if(obj.source != data_source_name) {
			...
			jQuery('button[aria-controls="attrib-'+obj.tabj3+'"]').css('display', 'none');
		} else  {
			jQuery('#'+obj.tab).parents().eq(2).css('display', 'block');
			jQuery('button[aria-controls="attrib-'+obj.tabj3+'"]').css('display', 'block');
		}
	});	
	...
	if(jQuery('.gk-used-option').length) {
		if(jQuery('.gk-used-option').attr('id') != 'jform_params_' + data_source_value) {
			...
			jQuery('.gk-used-option').parents('.control-group').css('display', 'none');
	...
	if(jQuery('#jform_params_' + data_source_value).length) {
		...
		jQuery('#jform_params_' + data_source_value).parents('.control-group').css('display', 'block');
	}
