...
class NSP_GK5_EVONEWS2 {
	...
	static function amount_of_articles($parent) {
		return ($parent->config['portal_mode_evonews2_num_leading'] + $parent->config['portal_mode_evonews2_num_intro']);
	}
	// output generator
	function output() {
		...
		// main wrapper
		echo '<div class="nspMain gkNspPM gkNspPM-EvoNews2" data-config="'.$news_config_json.'">';

		if(!empty($this->parent->config['nsp_pre_text']) && trim($this->parent->config['nsp_pre_text'])) {
			echo $this->parent->config['nsp_pre_text'];
		}

		$num_leading = $this->parent->config['portal_mode_evonews2_num_leading'];

		$num_intro = $this->parent->config['portal_mode_evonews2_num_intro'] + $num_leading;

		$columns_lead   = $this->parent->config['portal_mode_evonews2_leading_column'];

		$columns_intro  = $this->parent->config['portal_mode_evonews2_intro_column'];
      ...
      if ($num_intro>$num_leading && $i<$num_intro && $i>=$num_leading) {
		...
		if (!empty($leading_array)) {
            echo '<div class="leading columns-'.$columns_lead.'">';
            echo '<div class="row">';
        ...
        if (!empty($intro_array)) {
            echo '<div class="intro columns-'.$columns_intro.'">';
            echo '<div class="row">';
