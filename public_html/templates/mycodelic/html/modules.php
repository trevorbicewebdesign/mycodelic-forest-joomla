...
{ 
	...
	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-module module{$moduleClassSfx} {$moduleClass}\" id=\"Mod{$module->id}\" data-toggle=\"play-animation\" data-play=\"fadeIn\" data-offset=\"0\" data-delay=\"300\">" .
					"<div class=\"module-inner\">" . $badge;

		if ($module->showtitle != 0) {

			// double slesh for sub title
			$module_title_data =  explode('//', $module->title);

			if( is_array($module_title_data) ){
			    $module->title = '<span>'.trim($module_title_data[0]) . '</span>';
			    if( isset($module_title_data[1]) ){
			    $module->title .= '<span class="module-sub-title">' .$module_title_data[1]. '</span>';
			}
			} else {
...
{ 
	...
	if (!empty ($module->content)) {
		...
		if ($module->showtitle != 0) {
			...
			$html .= "<{$headerTag} class=\"row row-feature-title {$headerClass}\">{$module->title}</{$headerTag}>";
		}
