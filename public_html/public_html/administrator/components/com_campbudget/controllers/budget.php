...
class campbudgetControllerBudget extends JControllerForm {
	
	protected $text_prefix = 'COM_CAMPBUDGET_BUDGET';
	...
	public function save( $data )	{
		...
		if($input->get('task')=='apply') {
			if( !empty($_POST['return'])){
				$this->setRedirect('index.php?option=com_campbudget&view=budget'.'&layout=edit&id='.$row->id.'&return='.$_POST['return'], JText::_('COM_CAMPBUDGET_ACCOUNT_SAVED'));
			}
			else {
				$this->setRedirect('index.php?option=com_campbudget&view=budget'.'&layout=edit&id='.$row->id, JText::_('COM_CAMPBUDGET_ACCOUNT_SAVED'));
			}
		}
		else {
			if( !empty($_POST['return'])){
				...
				$this->setRedirect('index.php?option=com_campbudget&view=budgets', 'Budget Item Saved');
			}
		}
