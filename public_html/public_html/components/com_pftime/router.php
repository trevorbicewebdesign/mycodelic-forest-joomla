...
/**
 * Build the route for the com_pftime component
 *
    ...
    if($view == 'timesheet') {
        ...
        if (isset($query['filter_project'])) {
            if (strpos($query['filter_project'], ':') === false) {
                $query['filter_project'] = PFtimeMakeSlug($query['filter_project'], '#__pf_projects');
            }
        }
        else {
            $query['filter_project'] = PFtimeMakeSlug('0', '#__pf_projects');
        }
 ...
 */
function PFtimeParseRoute($segments)
{
    // Setup vars
    $vars  = array();
    ...
    if ($vars['view'] == 'timesheet') {
        if ($count == 1) {
            $vars['filter_project'] = PFtimeParseSlug($segments[0]);
        }
 ...
 */
function PFtimeParseSlug($segment)
{
 ...
 */
function PFtimeMakeSlug($id, $table, $alt = 'all', $field = 'alias')
{
    if ($id == '' || $id == '0') {
